...
export const makeMessagesSocket = (config: SocketConfig) => {
	...
	const createParticipantNodes = async (
	  ...
	  const patchedWithList = patched.map(pm => {
		...
		if (m.deviceSentMessage?.message?.listMessage) {
	  ...
	  const nodes = await Promise.all(
		patchedWithList.map(async ({ recipientJid: jid, ...patchedMessage }) => {
		  if (!jid) return {} as BinaryNode
	...
	const relayMessage = async (
		...
		await authState.keys.transaction(async () => {
			...
			if (isGroup || isStatus) {
				...
				if (single.deviceSentMessage?.message?.listMessage) {
				  single.deviceSentMessage.message.listMessage.listType = proto.Message.ListMessage.ListType.SINGLE_SELECT
				}
				if (single.listMessage) {
				  single.listMessage.listType = proto.Message.ListMessage.ListType.SINGLE_SELECT
				}
				const bytes = encodeWAMessage(single)
