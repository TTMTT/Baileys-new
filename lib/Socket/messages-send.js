...
const makeMessagesSocket = (config) => {
    ...
    const createParticipantNodes = async (jids, message, extraAttrs) => {
        ...
        const patchedWithList = patched.map(pm => {
            var _a, _b;
            const m = JSON.parse(JSON.stringify(pm));
            if ((_b = (_a = m.deviceSentMessage) === null || _a === void 0 ? void 0 : _a.message) === null || _b === void 0 ? void 0 : _b.listMessage) {
                m.deviceSentMessage.message.listMessage.listType = WAProto_1.proto.Message.ListMessage.ListType.SINGLE_SELECT;
            }
            if (m.listMessage) {
                m.listMessage.listType = WAProto_1.proto.Message.ListMessage.ListType.SINGLE_SELECT;
            }
            return m;
        });
        let shouldIncludeDeviceIdentity = false;
        const nodes = await Promise.all(patchedWithList.map(async ({ recipientJid: jid, ...patchedMessage }) => {
            if (!jid)
                return {};
    ...
    const relayMessage = async (jid, message, { messageId: msgId, participant, additionalAttributes, additionalNodes, useUserDevicesCache, useCachedGroupMetadata, statusJidList }) => {
        ...
        await authState.keys.transaction(async () => {
            ...
            if (isGroup || isStatus) {
                ...
                const single = JSON.parse(JSON.stringify(patched));
                if ((_c = (_b = single.deviceSentMessage) === null || _b === void 0 ? void 0 : _b.message) === null || _c === void 0 ? void 0 : _c.listMessage) {
                    single.deviceSentMessage.message.listMessage.listType = WAProto_1.proto.Message.ListMessage.ListType.SINGLE_SELECT;
                }
                if (single.listMessage) {
                    single.listMessage.listType = WAProto_1.proto.Message.ListMessage.ListType.SINGLE_SELECT;
                }
                const bytes = (0, Utils_1.encodeWAMessage)(single);
                ...
                for (const { user, device } of devices) {
                    ...
                    const jid = (0, WABinary_1.jidEncode)(isMe && isLid ? ((_e = (_d = authState.creds) === null || _d === void 0 ? void 0 : _d.me) === null || _e === void 0 ? void 0 : _e.lid.split(':')[0]) || user : user, isLid ? 'lid' : 's.whatsapp.net', device);
                    if (isMe) {
                        meJids.push(jid);
                    }
                    else {
            ...
            if (participants.length) {
                if ((additionalAttributes === null || additionalAttributes === void 0 ? void 0 : additionalAttributes['category']) === 'peer') {
                    const peerNode = (_g = (_f = participants[0]) === null || _f === void 0 ? void 0 : _f.content) === null || _g === void 0 ? void 0 : _g[0];
                    if (peerNode) {
